...
/**
 * shell_get_input - line less the newline
 * @inf: struct of parameter
 * Return: bytes read
 */
...
ssize_t shell_get_input(info_t *inf)
{
	...
	if (ln)
	{
		...
		*buff_ptr = p; /* pass back pointer to current command position */
		return (sh_strlen(p)); /* return length of current command */
	}
...
/**
 * shell_sigHandler - ctrl-C block
 * @sig_nb: signal number
 * Return: void
 */
...
/**
 * shell_getline - next line input
 * @inf: struct of parameter
 * @pt: pt to address buffer
